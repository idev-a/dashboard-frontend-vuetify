  ...
  export default {
    ...
    methods: {
      ...
      async deleteFeed () {
        ...
        this.loading = true
        try {
          const res = await Post(`${BASE_API}/api/admin/rss/delete`, this.editItem)
      ...
      async updateFeed () {
        ...
        try {
          const res = await Post(`admin/rss/update`, this.editItem)
          this.snackText = res.message
          this.snackColor = res.status
          if (res.status == 'success') {
      ...
      async readRss () {
        this.loading = true
        try {
          const res = await Post(`admin/rss/read`, { rss: this.link })
      ...
      async populateRss () {
        this.loading = true
        try {
          const res = await Post(`admin/rss/populate`, { rss: this.link })
          this.items = res.items
          this.snackText = res.message
          this.snackColor = res.status
          if (this.items.length == 0 && res.status == 'success') {
