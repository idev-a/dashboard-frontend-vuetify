<template>
  ...
  >
    ...
    <div v-if="select['ssllabs'] || false" id="scrapedPage">
      <v-card-title>
        <img src="https://ssllabs.com/images/qualys-ssl-labs-logo.png" width="341" height="55" alt="SSL Labs logo" title="SSL Labs logo">
      </v-card-title>
...
<script>
  import { BASE_API, fetchPublicData, getCompanyId, Post } from '@/api'
  ...
  export default {
    ...
    methods: {
      ...
      async fetchPage (data) {
        ...
        this.isIframe = false
        this.serviceLink = ''
        const res = await Post(`public`,  { url } )
        this.html = res.html.replace(/href="/gi, `href="${hostname}`).replace(/href='/gi, `href='${hostname}`)
        this.loading = false
      },
      onLoad (){
